...
class simpleapp_tk(Tkinter.Tk):
    ...
    def callback(self):
        ...
        face_size = 32
        selected = [7, 19, 26, 96, 129, 542, 3, 5, 87, 65]
        cut_encoded = x_test_encoded[np.array(selected)]
        cut_test = x_test[np.array(selected)]
	    ...
	    plt.title(str(i))
            plt.imshow(cut_test[i].reshape(face_size, face_size, 3), cmap='jet', interpolation='nearest')
